...
const MealTracker = () => {
    ...
    const handleAddFood = async () => {
        ...
        if (!isValidNumber(calories)) {
            ...
            alert('Calories must be a valid positive number');
            return;
        }
        if (!isValidNumber(protein)) {
            setAllFieldsComplete(false);
            setErrorMessage(ERROR_MESSAGES.INVALID_PROTEIN_ERROR);
            alert('Protein must be a valid positive number');
            return;
        }
        if (!isValidNumber(carbohydrates)) {
            setAllFieldsComplete(false);
            setErrorMessage(ERROR_MESSAGES.INVALID_CARBS_ERROR);
            alert('Carbohydrates must be a valid positive number');
            return;
        }
        if (!isValidNumber(fat)) {
            setAllFieldsComplete(false);
            setErrorMessage(ERROR_MESSAGES.INVALID_FAT_ERROR);
            alert('Fat must be a valid positive number');
            return;
        }
        if (!isValidNumber(servings)) {
            setAllFieldsComplete(false);
            setErrorMessage(ERROR_MESSAGES.INVALID_SERVINGS_ERROR);
            alert('Servings must be a valid positive number');
            return;
        }

        try {
            console.log('Adding food:', { foodName, calories, fat, protein, carbohydrates, servings, servingSize, mealType });
